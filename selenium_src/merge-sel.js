...
function stateAbb_to_state(obj)
{
    if(jsonStateAbb[obj["state"]] != undefined)
    {
        obj["state"] = jsonStateAbb[obj["state"]]

        if(obj["state"] === "District Of Columbia")
        {
            obj["state"] = "District of Columbia";
        }
    }
    else
    {
...
{
    let dateObj = new Date(obj["date"] + "/2020");
...
function merge_cases_county()
{
    for(let i = 0; i < jsonCases.length; i++)
    {
        ...
        else
        {
            ...
            if(county_dict[county_state] !== undefined && county_state !== "New York|New York")
            {
